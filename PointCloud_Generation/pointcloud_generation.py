...
class PointCloudGenerator:
    ...
    def generate_pointcloud(
        ...
        for box in boxes:
            ...
            target_boxes[box.phrase].append(box)
            print(box.box, box.logits, box.phrase)
        for target_box in target_boxes:
            if not target_boxes[target_box]:
                logger.error(f"Object {target_box} is not detected")
                raise ValueError
        # boxes = [target_boxes[target_name] for target_name in target_names]

        if self.sample_matcher is not None:
            boxes, scores = self.sample_matcher.filter_boxes(color_np_org, target_boxes)
            if not boxes:
